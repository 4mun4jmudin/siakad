...
class JurnalMengajarController extends Controller
{
    public function index(Request $request)
    {
        ...
        $jurnals = $baseQuery->latest('tanggal')->paginate(10)->withQueryString();

        $stats = [
            'total_jurnal' => (clone $statsQuery)->count(),
            'mengajar' => (clone $statsQuery)->where('status_mengajar', 'Mengajar')->count(),
            'digantikan' => (clone $statsQuery)->where('status_mengajar', 'Digantikan')->count(),
            'kosong' => (clone $statsQuery)->where('status_mengajar', 'Kosong')->count(),
        ];
    ...
    public function store(Request $request)
    {
        ...
        $validated = $request->validate([
            'id_jadwal' => [
                'required',
                'string',
                Rule::exists('tbl_jadwal_mengajar', 'id_jadwal'),
                Rule::unique('tbl_jurnal_mengajar')->where(function ($q) use ($request) {
                    return $q->where('tanggal', $request->tanggal)->whereNull('deleted_at');
                }),
            ],
    ...
    public function update(Request $request, JurnalMengajar $jurnalMengajar)
    {
        ...
        $validated = $request->validate([
            'id_jadwal' => [
                ...
                Rule::unique('tbl_jurnal_mengajar')->where(function ($q) use ($request) {
                    return $q->where('tanggal', $request->tanggal)->whereNull('deleted_at');
                })->ignore($jurnalMengajar->id_jurnal, 'id_jurnal'),
            ],
            'tanggal' => 'required|date|before_or_equal:today',
            'jam_masuk_kelas' => 'nullable|date_format:H:i',
            'jam_keluar_kelas' => 'nullable|date_format:H:i|after:jam_masuk_kelas',
        ...
        try {
            ...
            if ($conflict && $conflict->id_jurnal !== $jurnalMengajar->id_jurnal) {
                DB::rollBack();
                return back()->withErrors(['id_jadwal' => 'Jurnal untuk jadwal dan tanggal ini sudah ada.'])->withInput();
            }

            $jurnalMengajar->update(array_merge($validated, [
                'id_editor' => Auth::id(),
