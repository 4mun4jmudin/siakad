...
class JadwalMengajarController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        foreach ($timeSlots as $slot) {
            ...
            foreach (['Senin', 'Selasa', 'Rabu', 'Kamis', 'Jumat', 'Sabtu'] as $day) {
                // Gunakan filter() agar bisa mengambil semua jadwal di slot waktu ini
                ...
                $row[$day] = count($found) > 0 ? $found : null;
            }
    ...
    public function store(Request $request)
    {
        ...
        $id_jadwal = 'JDW-' . now()->format('ymdHis') . \Illuminate\Support\Str::random(4);
    ...
    public function updateTime(Request $request, JadwalMengajar $jadwalMengajar)
    {
        ...
        $newData = [
            'id_tahun_ajaran' => $jadwalMengajar->id_tahun_ajaran,
    ...
    public function jadwalPelajaran(Request $request)
    {
        ...
        // Query JadwalMengajar dengan relasi kelas, guru, mapel
        $query = JadwalMengajar::query()
            ->with([
                'guru:id_guru,nama_lengkap',
                'kelas:id_kelas,tingkat,jurusan',
        ...
        // Ambil data absensi siswa per mapel hari ini (status Hadir)
        $today = Carbon::today()->format('Y-m-d');
        $absensiHariIni = \App\Models\AbsensiSiswaMapel::where('tanggal', $today)
            ->where('status_kehadiran', 'Hadir')
            ->select('id_jadwal', DB::raw('count(*) as total_hadir'))
            ->groupBy('id_jadwal')
            ->get()
            ->keyBy('id_jadwal');

        // Tambahkan total siswa per kelas (relasi)
        $kelasIds = $jadwal->pluck('id_kelas')->unique();
        $siswaCountPerKelas = Kelas::whereIn('id_kelas', $kelasIds)
            ->withCount('siswa')
            ->get()
            ->keyBy('id_kelas');
        ...
        foreach ($jadwal as $j) {
            $hari = $j->hari;
            $kelasId = $j->id_kelas;
            $namaKelas = $j->kelas ? trim($j->kelas->tingkat . ' ' . $j->kelas->jurusan) : 'Kelas Dihapus';

            if (!isset($jadwalGrouped[$hari])) {
                $jadwalGrouped[$hari] = [];
            }
            if (!isset($jadwalGrouped[$hari][$namaKelas])) {
                $jadwalGrouped[$hari][$namaKelas] = [
                    'id_kelas' => $kelasId,
                    'nama_kelas' => $namaKelas,
                    'sesi' => [],
                    'total_siswa' => $siswaCountPerKelas[$kelasId]->siswa_count ?? 0,
                ];
            }

            $jArray = is_object($j) && method_exists($j, 'toArray') ? $j->toArray() : (array) $j;
            $jArray['hadir_hari_ini'] = $absensiHariIni[$j->id_jadwal]->total_hadir ?? 0;

            $jadwalGrouped[$hari][$namaKelas]['sesi'][] = $jArray;
        }

        $stats = [
            'total_sesi' => $jadwal->count(),
        ];

        return Inertia::render('admin/JadwalMengajar/JadwalPelajaran', [
            'jadwalGrouped' => $jadwalGrouped,
            'tahunAjaranAktif' => $tahunAjaranAktif,
