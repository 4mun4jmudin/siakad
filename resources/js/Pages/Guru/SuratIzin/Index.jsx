...
export default function Index({ auth, filters, surat }) {
  ...
  const canLampiranView = hasRoute('guru.surat-izin.lampiran.view');
  const canLampiranDownload = hasRoute('guru.surat-izin.lampiran.download');
  ...
  const resetFilters = () => {
    router.get(route('guru.surat-izin.index'), {}, { preserveState: false, replace: true, preserveScroll: true });
  };
  ...
  const submitAction = async () => {
    ...
    switch (confirm.type) {
      case 'approve':
        url = route('guru.surat-izin.approve', { surat: id });
        break;
      case 'reject':
        url = route('guru.surat-izin.reject', { surat: id });
        break;
      case 'resync':
        url = route('guru.surat-izin.resync', { surat: id });
        break;
      case 'unsync':
        url = route('guru.surat-izin.unsync', { surat: id });
        break;
      default:
        return;
    }
  ...
  const openPreview = (item) => {
    ...
    const viewUrl = canLampiranView ? route('guru.surat-izin.lampiran.view', { surat: item.id_surat }) : raw;
    const downloadUrl = canLampiranDownload
      ? route('guru.surat-izin.lampiran.download', { surat: item.id_surat })
      : raw;
  ...
  return (
    <GuruLayout user={auth.user} header="Surat Izin Siswa">
      ...
      <div className="relative min-h-screen overflow-hidden bg-gradient-to-br from-slate-50 via-indigo-50/40 to-sky-50/60 py-5 sm:py-6">
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex items-center gap-2">
            <Link
              href={route('guru.surat-izin.create')}
