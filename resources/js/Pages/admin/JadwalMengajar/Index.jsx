...
export default function Index({
    ...
    useEffect(() => {
        // Mencegah request berulang saat komponen pertama kali di-mount
        const isSame = 
            localFilters.filter_by === (filters?.filter_by || 'kelas') && 
            localFilters.kelas_id === (filters?.kelas_id || '') && 
            localFilters.guru_id === (filters?.guru_id || '');
                                        ...
                                        <tr key={rowIndex}>
                                            ...
                                            {daysOrder.map(day => (
                                                <td key={day} className="px-2 py-2 whitespace-nowrap text-sm text-gray-700 align-top">
                                                    {row[day] && Array.isArray(row[day]) ? (
                                                        <div className="flex flex-col gap-2 h-full">
                                                            {row[day].map((item, idx) => (
                                                                <div key={idx} className="bg-gray-50 p-3 rounded-md border-l-4 group relative" style={{ borderColor: generateColor(item.guru?.nama_lengkap) }}>
                                                                    <div className="absolute top-2 right-2 flex gap-x-1 opacity-0 group-hover:opacity-100 transition-opacity z-10">
                                                                        <button onClick={() => openDetailModal(item)} className="p-1 rounded-full bg-blue-50 text-blue-600 hover:bg-blue-100" title="Lihat Detail"><EyeIcon className="h-4 w-4"/></button>
                                                                        <button onClick={() => openEditModal(item)} className="p-1 rounded-full bg-yellow-50 text-yellow-600 hover:bg-yellow-100" title="Edit Jadwal"><PencilIcon className="h-4 w-4"/></button>
                                                                        <button onClick={() => openDeleteModal(item)} className="p-1 rounded-full bg-red-50 text-red-600 hover:bg-red-100" title="Hapus Jadwal"><TrashIcon className="h-4 w-4"/></button>
                                                                    </div>
                                                                    <p className="font-bold text-gray-900 text-xs leading-tight pr-14">{item.mapel?.nama_mapel || 'Mapel Dihapus'}</p>
                                                                    <p className="text-xs text-gray-500 italic mt-1">{item.guru?.nama_lengkap || 'Guru Dihapus'}</p>
                                                                    {((filters.filter_by === 'guru' && !localFilters.guru_id) || (filters.filter_by === 'kelas' && !localFilters.kelas_id) || filters.filter_by === 'guru') && <p className="text-xs text-indigo-600 mt-1 font-semibold">{item.kelas?.nama_lengkap || 'Kelas Dihapus'}</p>}
                                                                </div>
                                                            ))}
                                                        </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div className="space-y-4">
                            ...
                            <div>
                                ...
                                <select id="id_mapel" className="mt-1 block w-full border-gray-300 rounded-md shadow-sm" value={data.id_mapel} onChange={(e) => {
                                    const selectedId = e.target.value;
                                    setData('id_mapel', selectedId);
                                    
                                    // Auto-fill guru pengampu default jika ada
                                    if (selectedId) {
                                        const selectedMapel = mapelOptions.find(m => m.id_mapel === selectedId);
                                        if (selectedMapel && selectedMapel.id_guru_default) {
                                            setData(data => ({...data, id_mapel: selectedId, id_guru: selectedMapel.id_guru_default}));
                                        }
                                    }
                                }}>
