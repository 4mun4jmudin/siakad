...
import { Users, Wifi, Clock, XCircle, MapPin, Search } from 'lucide-react';
...
export default function AdminLiveLocationSiswa({ auth, sekolah }) {
    ...
    const filteredStudents = searchTerm === '' 
        ...
        : locations.filter(loc => 
            (loc.nama_lengkap?.toLowerCase().includes(searchTerm.toLowerCase()) || 
             loc.nis?.includes(searchTerm))
        );

    const handleSelectStudent = (student) => {
        setSearchTerm(student.nama_lengkap);
        setShowDropdown(false);
        if (mapRef.current && student.latitude && student.longitude) {
            mapRef.current.flyTo([student.latitude, student.longitude], 18, {
                duration: 1.5
            });
        }
    };
        ...
        <AdminLayout user={auth.user} header="Live Lokasi Siswa" subtitle="Pemantauan realtime GPS siswa aktif di area sekolah.">
            ...
            <div className="py-6 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto space-y-6">
                ...
                <div className="bg-white rounded-[2rem] shadow-sm border border-slate-200 overflow-hidden relative">
                    <div className="absolute top-4 left-4 z-[400] w-64 sm:w-80">
                        <div className="relative">
                            ...
                            <input
                                type="text"
                                className="block w-full pl-10 pr-3 py-2.5 border border-slate-200 rounded-xl leading-5 bg-white/90 backdrop-blur-sm shadow-sm placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-cyan-500 focus:border-cyan-500 sm:text-sm transition-all"
                        ...
                        {showDropdown && filteredStudents.length > 0 && (
                            <div className="absolute mt-2 w-full bg-white/95 backdrop-blur-md rounded-xl shadow-xl border border-slate-100 overflow-hidden max-h-64 overflow-y-auto z-[410]">
                                {filteredStudents.map((student) => (
                                    <div 
                                        key={student.id_siswa} 
                                        className="px-4 py-3 hover:bg-slate-50 cursor-pointer border-b border-slate-100 last:border-0 flex flex-col transition-colors"
                                        onClick={() => handleSelectStudent(student)}
                                    >
                                        <div className="flex items-center gap-2">
                                            <div className={`w-2 h-2 rounded-full ${student.status === 'online' ? 'bg-emerald-500' : student.status === 'idle' ? 'bg-amber-500' : 'bg-rose-500'}`}></div>
                                            <span className="text-sm font-bold text-slate-800">{student.nama_lengkap}</span>
                                        </div>
                                        <span className="text-[11px] font-semibold text-slate-500 ml-4">{student.nis} • {student.kelas}</span>
                                    </div>
                                ))}
                            </div>
                        )}
                        {showDropdown && searchTerm !== '' && filteredStudents.length === 0 && (
                            <div className="absolute mt-2 w-full bg-white/95 backdrop-blur-md rounded-xl shadow-xl border border-slate-100 px-4 py-3 text-sm font-medium text-slate-500 text-center z-[410]">
