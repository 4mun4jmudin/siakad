            ...
            <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                        ...
                        <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
                            <div className="flex items-center gap-4 mb-4">
                                ...
                                <div>
                                    ...
                                    <p className="text-sm text-gray-500">{wali.hubungan} dari {wali.siswas?.length || 0} Siswa</p>
                                </div>
                            </div>
                    ...
                    <div className="lg:col-span-2 space-y-6">
                        {wali.siswas && wali.siswas.length > 0 ? wali.siswas.map((siswa, idx) => (
                             <div key={idx} className="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
                                <div className="flex items-center gap-4 mb-4">
                                    <AcademicCapIcon className="h-12 w-12 text-gray-400"/>
                                    <div>
                                        <h3 className="text-lg font-bold text-gray-800">{siswa.nama_lengkap}</h3>
                                        <p className="text-sm text-gray-500">Siswa Perwalian {idx + 1}</p>
                                    </div>
                                </div>
                                <div className="grid grid-cols-2 gap-x-6 gap-y-2 text-sm">
                                    <DetailRow label="NIS" value={siswa.nis} />
                                    <DetailRow label="NISN" value={siswa.nisn} />
                                    <DetailRow label="Kelas" value={siswa.kelas ? `${siswa.kelas.tingkat} ${siswa.kelas.jurusan}` : 'Belum Ada Kelas'} />
                                    <DetailRow label="Status Siswa" isBadge={true}><StatusBadge status={siswa.status || 'N/A'} /></DetailRow>
                                </div>
                            </div>
                        )) : (
                             <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200 text-center text-gray-500 py-8">
                                Belum ada siswa perwalian yang ditambahkan.
                             </div>
                        )}
                        <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
                            ...
                            <div className="overflow-x-auto">
                                <table className="min-w-full">
                                    <thead className="bg-gray-50">
                                        <tr>
                                            {['Siswa', 'Tanggal', 'Status', 'Jam Masuk', 'Keterangan'].map(head => (
                                        ...
                                        {absensiSiswa.length > 0 ? absensiSiswa.map(absen => (
                                            <tr key={absen.id_absensi}>
                                                <td className="px-4 py-3 whitespace-nowrap text-sm font-medium text-gray-900">
                                                    {wali.siswas?.find(s => s.id_siswa === absen.id_siswa)?.nama_lengkap || 'Unknown'}
                                                </td>
                                            ...
                                            <tr><td colSpan="5" className="text-center py-8 text-gray-500">Belum ada riwayat absensi.</td></tr>
                                        )}
                                    </tbody>
                                </table>
                            </div>
