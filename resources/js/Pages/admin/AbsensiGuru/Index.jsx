...
const StatusBadge = ({ status, metode = 'Auto' }) => {
  const isManual = metode === 'Manual Admin';
  const styles = {
    Hadir: isManual ? 'bg-indigo-100 text-indigo-800 border border-indigo-200' : 'bg-green-100 text-green-800 border border-green-200',
    Sakit: 'bg-yellow-100 text-yellow-800 border border-yellow-200',
    Izin: 'bg-blue-100 text-blue-800 border border-blue-200',
    Alfa: 'bg-red-100 text-red-800 border border-red-200',
    'Dinas Luar': 'bg-purple-100 text-purple-800 border border-purple-200',
  };
  return (
    <span className={`px-2.5 py-1 text-xs font-semibold rounded-full shadow-sm ${styles[status] || 'bg-gray-100 text-gray-800'}`}>
      {status} {status === 'Hadir' && isManual ? '(Manual)' : ''}
    </span>
  );
};

const MetodeBadge = ({ metode }) => {
  if (metode === 'Manual Admin') {
    return <span className="px-2 py-1 text-[10px] uppercase font-bold tracking-wider rounded bg-indigo-50 text-indigo-700 border border-indigo-200">Manual Admin</span>;
  }
  return <span className="px-2 py-1 text-[10px] uppercase font-bold tracking-wider rounded bg-gray-50 text-gray-500 border border-gray-200">Sistem (Auto)</span>;
};

// Helper function to extract method and actual reasoning from the database's "keterangan" field
export const parseKeterangan = (rawKeterangan) => {
  if (!rawKeterangan) return { metode: 'Auto', text: '' };
  
  if (rawKeterangan.startsWith('[MANUAL_ADMIN]')) {
    // If it starts with the flag, it's a manual override
    const text = rawKeterangan.replace('[MANUAL_ADMIN]', '').trim();
    return { metode: 'Manual Admin', text };
  }
  
  return { metode: 'Auto', text: rawKeterangan };
};
...
const TabLink = ({ href, isActive, children }) => (
  <Link
    ...
    preserveState
    preserveScroll
    className={`px-3 py-2 font-semibold text-sm rounded-md ${isActive ? 'bg-white text-blue-600 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}
  >
    {children}
  </Link>
);
...
export default function Index({
  ...
  const [uiFilterMetode, setUiFilterMetode] = useState('Semua'); // Semua, Auto, Manual Admin
  const [uiFilterStatus, setUiFilterStatus] = useState('Semua'); // Semua, Hadir, Izin, Sakit, Alfa
  ...
  const openModal = (absen = null) => {
    if (absen) {
      setIsEditMode(true);
      const parsed = parseKeterangan(absen.keterangan);
      setData({
        tanggal: filters.tanggal,
        ...
        keterangan: parsed.text || '',
        alasan_manual: parsed.metode === 'Manual Admin' ? parsed.text : '',
      });
    } else {
      setIsEditMode(false);
      reset();
  ...
  const submitManualAbsensi = (e) => {
    ...
    // Inject the MANUAL_ADMIN flag into keterangan
    const finalKeterangan = data.alasan_manual 
      ? `[MANUAL_ADMIN] ${data.alasan_manual}` 
      : `[MANUAL_ADMIN] Input/Override by Admin`;
      
    router.post(route('admin.absensi-guru.store'), {
      ...data,
  ...
  const handleExport = (format) => {
    ...
    exportUrl = format === 'excel' ? route('admin.absensi-guru.export-excel') : route('admin.absensi-guru.export-pdf');
    ...
    <AdminLayout user={auth.user} header="Absensi Guru">
      ...
      <div className="space-y-8">
        <div className="flex justify-between items-center">
          ...
          <button
            onClick={() => openModal()}
            className="flex items-center bg-blue-600 text-white px-4 py-2 rounded-lg shadow-sm hover:bg-blue-700 transition"
          >
        ...
        {filters.tab === 'harian' && stats && (() => {
          // Calculate frontend stats for Hadir (Auto vs Manual) based on absensiData
          const hadirAutoCount = absensiData?.filter(a => a.status_kehadiran === 'Hadir' && parseKeterangan(a.keterangan).metode === 'Auto').length || 0;
          const hadirManualCount = absensiData?.filter(a => a.status_kehadiran === 'Hadir' && parseKeterangan(a.keterangan).metode === 'Manual Admin').length || 0;
          ...
          return (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-6">
              ...
              <StatCard
                title="Hadir (Auto)"
                value={hadirAutoCount}
                description="Input dari sistem"
                icon={<CheckCircleIcon className="h-8 w-8 text-green-500" />}
                color="#22c55e"
              />
              <StatCard
                title="Hadir (Manual)"
                value={hadirManualCount}
                description="Input dari admin"
                icon={<ExclamationTriangleIcon className="h-8 w-8 text-indigo-500" />}
                color="#6366f1"
              />
        ...
        <div className="bg-gray-100 p-1 rounded-lg flex space-x-2">
          <TabLink href={route('admin.absensi-guru.index', { tab: 'harian' })} isActive={filters.tab === 'harian'}>
            Absensi Hari Ini
          </TabLink>
          <TabLink href={route('admin.absensi-guru.index', { tab: 'riwayat' })} isActive={filters.tab === 'riwayat'}>
            Riwayat Absensi
          </TabLink>
          <TabLink href={route('admin.absensi-guru.index', { tab: 'laporan_bulanan' })} isActive={filters.tab === 'laporan_bulanan'}>
            Laporan Bulanan
          </TabLink>
          <TabLink href={route('admin.absensi-guru.index', { tab: 'laporan_semester' })} isActive={filters.tab === 'laporan_semester'}>
            Laporan Semester
          </TabLink>
        </div>
        ...
        <div>
          {filters.tab === 'harian' && (() => {
            const filteredAbsensiData = absensiData?.map(absen => {
              const { metode, text } = parseKeterangan(absen.keterangan);
              return { ...absen, metode, keteranganAsli: text };
            }).filter(absen => {
              if (uiFilterMetode !== 'Semua' && absen.metode !== uiFilterMetode) return false;
              if (uiFilterStatus !== 'Semua' && absen.status_kehadiran !== uiFilterStatus) return false;
              return true;
            });

            return (
              <div className="space-y-6 animate-fade-in">
                {chartData && <WeeklyAttendanceChart chartData={chartData} />}

                <div className="bg-white p-6 rounded-lg shadow-sm">
                  <h3 className="text-lg font-bold text-gray-800 mb-4">Filter Data</h3>
                  <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                    <div>
                      <InputLabel value="Cari Guru / NIP" />
                      <TextInput type="text" defaultValue={filters.search} onChange={handleSearch} placeholder="Ketik nama/NIP..." className="w-full mt-1" />
                    </div>
                    <div>
                      <InputLabel value="Tanggal" />
                      <TextInput type="date" value={filters.tanggal} onChange={(e) => handleFilterChange('tanggal', e.target.value)} className="w-full mt-1" />
                    </div>
                    <div>
                      <InputLabel value="Metode Absen" />
                      <select value={uiFilterMetode} onChange={(e) => setUiFilterMetode(e.target.value)} className="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                    ...
                    <div>
                      <InputLabel value="Status Kehadiran" />
                      <select value={uiFilterStatus} onChange={(e) => setUiFilterStatus(e.target.value)} className="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                ...
                <div className="bg-white p-6 rounded-lg shadow-sm">
                  <h3 className="text-lg font-bold text-gray-800 mb-4 flex items-center justify-between">
                    <span>
                      Daftar Absensi Guru -{' '}
                      {new Date(filters.tanggal + 'T00:00:00').toLocaleDateString('id-ID', {
                        weekday: 'long',
                        year: 'numeric',
                        month: 'long',
                        day: 'numeric',
                    ...
                    <span className="text-sm font-normal text-gray-500 bg-gray-100 px-3 py-1 rounded-full">
                      Total Ditampilkan: {filteredAbsensiData?.length || 0}
                    </span>
                  </h3>
                  <div className="overflow-x-auto">
                    <table className="min-w-full divide-y divide-gray-200">
                      <thead className="bg-gray-50">
                        <tr>
                          {['NIP', 'Nama Guru', 'Jam Masuk', 'Metode', 'Status', 'Keterlambatan', 'Aksi'].map((head) => (
                      ...
                      <tbody className="bg-white divide-y divide-gray-200">
                        {filteredAbsensiData?.map((absen) => (
                          <tr key={absen.id_guru} className={absen.metode === 'Manual Admin' ? 'bg-indigo-50/30' : ''}>
                            ...
                            <td className="px-6 py-4 whitespace-nowrap">
                              <MetodeBadge metode={absen.metode} />
                              {absen.metode === 'Manual Admin' && <p className="text-[10px] text-gray-400 mt-1">Diinput oleh Admin</p>}
                            </td>
                            <td className="px-6 py-4 whitespace-nowrap">
                              <StatusBadge status={absen.status_kehadiran} metode={absen.metode} />
                            </td>
                          ...
                          <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                            <div className="flex items-center gap-x-3">
                              <Link
                                href={route('admin.absensi-guru.show', { absensi_guru: absen.id_guru })}
            ...
            <div className="bg-white p-6 rounded-lg shadow-sm animate-fade-in">
              ...
              <div className="overflow-x-auto">
                <table className="min-w-full divide-y divide-gray-200">
                  <thead className="bg-gray-50">
                    <tr>
                      {['Tanggal', 'Nama Guru', 'Metode', 'Status', 'Jam Masuk', 'Jam Pulang', 'Keterangan'].map((head) => (
                        <th key={head} className="px-6 py-3 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">
                          {head}
                        </th>
                      ))}
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {riwayatAbsensi?.data?.map((absen) => {
                      const { metode, text } = parseKeterangan(absen.keterangan);
                      return (
                        <tr key={absen.id_absensi} className={metode === 'Manual Admin' ? 'bg-indigo-50/30' : ''}>
                          ...
                          <td className="px-6 py-4">
                            ...
                            {metode === 'Manual Admin' && <p className="text-[10px] text-gray-400 mt-1">Diinput oleh Admin</p>}
                          </td>
                          <td className="px-6 py-4">
                            <StatusBadge status={absen.status_kehadiran} metode={metode} />
                          </td>
            ...
            <div className="bg-white p-6 rounded-lg shadow-sm animate-fade-in">
              <div className="flex flex-wrap justify-between items-center gap-4 mb-4">
                ...
                <div className="flex items-center gap-4">
                  ...
                  <button
                    ...
                    className="flex items-center bg-red-600 text-white px-3 py-2 rounded-lg shadow-sm hover:bg-red-700 transition text-sm"
                  >
                    <PrinterIcon className="h-5 w-5 mr-2" /> PDF
                  </button>
                  <button
                    onClick={() => handleExport('excel')}
                    className="flex items-center bg-green-600 text-white px-3 py-2 rounded-lg shadow-sm hover:bg-green-700 transition text-sm"
                  >
              ...
              <div className="overflow-x-auto">
                <table className="min-w-full divide-y divide-gray-200">
                  <thead className="bg-gray-50">
                    <tr>
                      {['Nama Guru', 'Hadir', 'Sakit', 'Izin', 'Alfa', 'Dinas Luar', 'Kehadiran (%)', 'Rata Telat (mnt)', 'Aksi'].map((head) => (
                        <th key={head} className="px-6 py-3 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">
                          {head}
                        </th>
                      ))}
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {laporanBulanan?.map((guru) => (
                      <tr key={guru.id_guru}>
                        ...
                        <td className="px-6 py-4 text-center">{guru.dinas_luar ?? 0}</td>
                        <td className="px-6 py-4 text-center">{guru.persen_hadir ?? 0}%</td>
                        <td className="px-6 py-4 text-center">{guru.rata_telat ?? 0}</td>
                        <td className="px-6 py-4">
                          <Link
                            href={route('admin.absensi-guru.show', {
                              absensi_guru: guru.id_guru,
                              bulan: filters.bulan,
                              tahun: filters.tahun,
                            })}
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {laporanSemesteran?.map((guru) => (
                      <tr key={guru.id_guru}>
                        ...
                        <td className="px-6 py-4">
                          <Link
                            href={route('admin.absensi-guru.show', { absensi_guru: guru.id_guru })}
                            className="inline-flex items-center gap-1 text-blue-600 hover:text-blue-800"
                          >
                            <EyeIcon className="h-5 w-5" />
                            Detail
      ...
      <Modal show={isModalOpen} onClose={closeModal}>
        <form onSubmit={submitManualAbsensi} className="p-6">
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <select
                ...
                className="mt-1 block w-full border-gray-300 rounded-md shadow-sm"
              >
                {['Hadir', 'Sakit', 'Izin', 'Alfa', 'Dinas Luar'].map((opt) => (
            ...
            <div>
              <InputLabel htmlFor="alasan_manual" value="Alasan Input Manual / Override" />
              <TextInput
                id="alasan_manual"
                value={data.alasan_manual}
                onChange={(e) => setData('alasan_manual', e.target.value)}
                className="mt-1 block w-full border-indigo-300 focus:border-indigo-500 focus:ring-indigo-500"
                placeholder="Contoh: Lupa absen, error sistem, dinas mendadak..."
                required
              />
              <p className="text-[11px] text-gray-500 mt-1 flex items-center gap-1">
                <ExclamationTriangleIcon className="h-3 w-3 text-amber-500" />
                Absensi ini akan dicatat sebagai input manual (override) oleh Admin.
              </p>
            </div>
            <div>
              <InputLabel htmlFor="keterangan" value="Keterangan Tambahan (Opsional)" />
          ...
          <div className="mt-6 flex justify-end">
            ...
            <PrimaryButton className="ml-3" disabled={processing || !data.id_guru || !data.status_kehadiran || !data.alasan_manual}>
