...
import { Head, Link, useForm, usePage, router } from '@inertiajs/react'; // Tambahkan 'router' di sini
...
const Create = ({ auth, jadwalOptions, guruOptions, jadwalId }) => {
    ...
    useEffect(() => {
        ...
        if (selectedJadwal) {
            setData(prev => ({
                ...prev,
                jam_masuk_kelas: selectedJadwal.jam_mulai?.slice(0, 5),
                jam_keluar_kelas: selectedJadwal.jam_selesai?.slice(0, 5),
            }));
        } else {
            setData(prev => ({
                ...prev,
    ...
    const submit = (e) => {
        e.preventDefault();
        post(route('admin.jurnal-mengajar.store'));
    };

    return (
        <AdminLayout user={auth.user} header="Tambah Jurnal Mengajar">
            <Head title="Tambah Jurnal Mengajar" />
            <div className="max-w-4xl mx-auto space-y-6">
                <div className="bg-white p-6 rounded-lg shadow-sm">
                    <h2 className="text-xl font-bold text-gray-800 mb-6">Formulir Jurnal Baru</h2>
